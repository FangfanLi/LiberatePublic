...
class Sender(object):
    ...
    '''
    def __init__(self, mpacNum, action, spec):
...
def run(configs = Configs(), pcapdir = None, cmpacNum = -1, caction = None, cspec = None, smpacNum = -1, saction = None, sspec = None, testID = '0', byExternal = False, libProxy=None):
    ...
    senderObj = Sender(cmpacNum, caction, cspec)
    ...
    if libProxy != None:
        PRINT_ACTION('Adding iptables rule, starting Liberate Proxy: ' + libProxy.ChangeCode, 0)
        # ADD iptables rule and start liberateProxy here
    ...
    # Remove iptables rules and stop liberateProxy here
        PRINT_ACTION('Removing iptables rule, Stopping Liberate Proxy: ' + libProxy.ChangeCode, 0)
